...
test("writes off in .env leaves the server with the 11 read tools only", async () => {
  const dataDir = mkdtempSync(join(tmpdir(), "wazap-readonly-"));
  await run(process.execPath, [binary, "config", "writes", "off", "--data-dir", dataDir]);
  const { toolNames } = await runSmoke({ args: ["serve"], dataDir, keepDataDir: true, expectedTools: 11, expectReadOnly: true });
