...
const CASES = [
  ...
  {
    name: "explicit serve at a terminal serves",
    config: { explicitCommand: true },
    stdin: true,
    stderr: true,
    expect: "serve",
  },
  ...
  {
    name: "another command is never greeted",
    config: { command: "login", explicitCommand: true },
    stdin: true,
    stderr: true,
    expect: "serve",
  },
...
test("writes off in .env leaves the server with the 23 read tools only", async () => {
