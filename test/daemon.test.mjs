...
test("--http publishes its own port and takes the internal token as a full-access bearer", async () => {
  await withDaemon({}, async ({ daemonFile }) => {
    ...
    assert.equal(await httpToolCount(info.port, null), 13, "an anonymous session gets the read tools");
    assert.equal(await httpToolCount(info.port, info.token), 24, "the internal token gets everything");
  }, ["serve", "--http", "--port", "0"]);
});
