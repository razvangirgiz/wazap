...
2. `recall` when it is on: it matches by meaning, not exact words, so a paraphrase or another language still hits, and it reaches messages too old for `search_messages`. Each hit carries its date and a score; "index only" means the message left the live store — quote it, but `get_message` and `download_media` cannot see it. For an exact string (an id, a number, a URL) or when `recall` answers RECALL_UNAVAILABLE, use `search_messages` with the user's words. If nothing matches, run two more variants before giving up: a synonym or the other language the user writes in, and a narrower fragment (a street name instead of "the address", "factura" instead of "the invoice from March").
