...
| `✗ credentials` | Unreadable. `npx wazap logout` then `npx wazap login`. |
...
`npx wazap status --live` reaches WhatsApp for real and reports whether the
...
Run `npx wazap connect <client>`, where the client is one of `claude-code`,
...
Turn them on with `npx wazap config writes on`, off again with
`npx wazap config writes off`. Both edit `WAZAP_READ_ONLY` in
`<data-dir>/.env`; a running server has to be restarted for the change to take
effect. `npx wazap config` alone prints every effective setting and where it
