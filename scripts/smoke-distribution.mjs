...
const transport = new StdioClientTransport({
  ...
  args: docker
    ? [
        ...
        ...(multi ? ["-v", `${join(dir, "accounts.json")}:/data/accounts.json:ro`] : []),
        target,
        "serve",
      ]
...
try {
  ...
  if (multi) {
    ...
    assert.equal(accounts.structuredContent.default, ids[0]);
    for (const account_id of ids)
      assert.equal(
        (await client.callTool({ name: "get_status", arguments: { account_id } })).structuredContent.status,
        "not_linked"
      );
  }
