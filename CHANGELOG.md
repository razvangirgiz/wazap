...
- **What an assistant is sent is 4,753 tokens instead of 14,397**: the names,
  descriptions, input schemas and annotations of a session that can write, as
  an SDK client lists them (2,915 in a read session). Every description fits in
  300 characters, `account_id` is explained once, in the server's instructions
  and in `learn`, and `learn`'s guide is 1,851 tokens of text with no copy in
